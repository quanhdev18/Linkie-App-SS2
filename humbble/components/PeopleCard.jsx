...
const PeopleCard = ({ filterData }) => {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      // ⬇️ LẤY PROFILE GIỐNG WEB (KHÔNG LẤY LOCATION RIÊNG)
      const [allProfiles, likedUsers] = await Promise.all([
        getProfiles(token),
        getLikedUsers(parsedId),
      ]);
      ...
      let filteredProfiles = allProfiles.filter(
        (user) => user.account_id !== parsedId && !likedIds.has(user.account_id)
      );
      ...
      if (filterData) {
        filteredProfiles = filteredProfiles.filter((user) => {
          ...
          const ageMatch = age >= filterData.minAge && age <= filterData.maxAge;
          const genderMatch =
            !filterData.gender || filterData.gender.length === 0 || filterData.gender.includes(user.gender);
          return ageMatch && genderMatch;
        });
      }
      ...
      // ⬇️ KHÔNG FETCH LOCATION → Lấy luôn từ profile
      filteredProfiles = filteredProfiles.map((user) => ({
        ...user,
        ...
        return (
          <Swiper
            ...
            onSwipedRight={async (index) => {
              ...
              if (likedUser) {
                const likerId = await AsyncStorage.getItem("account_id");
                if (likerId) await likeUser(likedUser.account_id, parseInt(likerId));
              }
            }}
            infinite
            backgroundColor="transparent"
...
const styles = StyleSheet.create({
  ...
  statusText: { fontSize: 16, color: "#666", marginBottom: 15, textAlign: "center" },
  // card: {
  //   width: width * 0.9,
  //   height: height * 0.7,
  //   borderRadius: 10,
  ...
  infoSection: { width: "100%", padding: 12, backgroundColor: "rgba(0,0,0,0.6)" },
  text: { color: "white", fontSize: 24, fontWeight: "bold" },
  locationText: { color: "white", fontSize: 16, marginTop: 4 },
  overlayWrapper: { justifyContent: "center", alignItems: "center" },
});
