...
// ⚠️ IP MÁY CHẠY FASTAPI (nên đồng bộ với log Metro)
const REAL_DEVICE_IP = "192.168.0.109";

const API_IP = Device.isDevice ? REAL_DEVICE_IP : "10.0.2.2";
const WS_IP = Device.isDevice ? REAL_DEVICE_IP : "10.0.2.2";

const baseURL = `http://${API_IP}:8000`;
export const WS_BASE_URL = `ws://${WS_IP}:8000`;
export const BASE_URL = baseURL;
...
api.interceptors.request.use(
  async (config) => {
    ...
    if (token) {
      // ✅ FIX TYPE — BẮT BUỘC CHO AXIOS V1+
      config.headers = {
        ...(config.headers || {}),
...
export const getProfileByAccountId = async (accountId) => {
  const res = await api.get(`/profiles/by-account/${accountId}`);
  return res.data;
};
...
// 🔥 Get avatar + profile images theo accountId
...
export const fetchPackagesApi = () => {
  return api.get("/packages");
};

// 💳 Tạo payment intent (Visa)
export const createVisaPayment = (packageId) => {
  return api.post("/payment/visa", {
    package_id: packageId,
  });
};

// ✅ Callback xác nhận thanh toán
export const confirmVisaPayment = () => {
  return api.get("/payment/visa-callback");
};
...
// };
export const updateLocation = async (accountId, lat, lng) => {
  await api.post(`/location/update_location/${accountId}`, {
    latitude: lat,
    longitude: lng,
  });
};
