...
export default function ChatScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      //     // Nhận tin NHƯ WEB
      //     if (msg.from_user_id == toUserId || msg.from_user_id == userId) {
      //       setMessages((prev) => [...prev, msg]);
      //     }
      //   } catch (e) {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (
            ...
            (msg.from_user_id == userId || msg.from_user_id == toUserId)
          ) {
            setMessages((prev) => [...prev, msg]);
          }
        } catch (e) {
  ...
  // const handleCallSignal = (msg) => {
  //   switch (msg.type) {
  //     case "CALL_INVITE":
  //       Alert.alert("Cuộc gọi đến", `${toUsername} đang gọi bạn`, [
  //         {
  //           text: "Từ chối",
  //           onPress: () => {
  //             socket.send(
  ...
  //       to_user_id: parseInt(toUserId),
  //       call_type: type,
  //       room_id: `chat_${userId}_${toUserId}`,
  //     })
  //   );
  // };
  const handleCallSignal = (msg) => {
      ...
      Alert.alert("Cuộc gọi đến", `${toUsername} đang gọi bạn`, [
        ...
        },
        {
          ...
          onPress: () => {
            ...
            // ✅ FIX: chỉ PUSH – JOIN ZEGO làm trong Call Screen
            router.push({
              pathname:
                msg.call_type === "video"
                  ? "/(calls)/VideoCall"
  ...
  const startCall = (type) => {
    // ✅ FIX: check socket ready
    if (!socket || socket.readyState !== WebSocket.OPEN) {
      Alert.alert("Đang kết nối", "Vui lòng thử lại sau 1 giây");
      return;
    }
